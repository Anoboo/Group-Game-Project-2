...
public class Waves : MonoBehaviour
{
    ...
    public class Wave
    {
        ...
        public Transform speederZombie;
        public int count;
    ...
    IEnumerator SpawnWave(Wave _wave)
    {
        ...
        for(int i = 0; i < _wave.count; i++)
        {
            ...
            if (nextWave == 3 && Random.value <= 0.6)
            {
                SpawnEnemy(_wave.enemy);
            }
            if (nextWave == 3 && Random.value > 0.75)
            {
                SpawnEnemyBeefCake(_wave.beefCake);
            }
            if (nextWave == 3 && Random.value > 0.85)
            {
                SpawnEnemySpeedZombie(_wave.speederZombie);
            }
    ...
    void SpawnEnemySpeedZombie(Transform _SpeederZombie)
    {
        Debug.Log("Spawning Speeder!" + _SpeederZombie.name);
        Transform _sp = spawnPoints[Random.Range(0, spawnPoints.Length)];
        Instantiate(_SpeederZombie, _sp.position, transform.rotation * Quaternion.Euler(0f, 180f, 0f));
    }
